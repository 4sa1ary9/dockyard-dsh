...
`/dockyard login` opens the provider's official browser authorization page and imports the completed account into the pool. Codex, Antigravity, Grok, Claude, and Cursor use provider-verified browser flows; CLI/desktop/OAuth-file sources remain compatibility or scan fallbacks. Kiro uses the official `kiro-cli login` flow and keeps its credential in Kiro's own secure store; Dockyard stores only an opaque active-session marker. Claude can request a manual authorization-code paste when its official hosted callback does not return to localhost. The GUI Login/Add action always starts a new authorization flow, even when accounts already exist, instead of silently re-importing the active account. Use `/dockyard scan` plus `/dockyard add` only when importing an existing local session. Completed credentials are written to the host secure store and only opaque references reach the account pool or page state.
...
The Codex module uses the locally imported OAuth account and the native Codex Responses transport when the DSH pi-ai dependencies are present. Antigravity exposes official session discovery, quota, credits, and live model catalog, while generation uses the provider's native Gemini `streamGenerateContent?alt=sse` transport. Claude, Cursor, and Grok use their provider-native streaming adapters. Kiro launches the official `kiro-cli acp` agent and communicates over Agent Client Protocol JSON-RPC on stdio; its live catalog comes from `kiro-cli chat --list-models --format json`. Cursor browser accounts resolve identity through Cursor's official AuthService and load the account model catalog through its official AvailableModels RPC; the CLI remains a compatibility fallback. Claude/Cursor quota is only shown when their official session status contains real windows; Grok and Kiro currently expose no dependable structured subscription quota command, so Dockyard leaves those fields unknown.
